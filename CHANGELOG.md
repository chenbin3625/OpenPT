...
本版新增异常种子自动归档功能，并完善相关配置与前端展示。

### 新增特性

- **异常种子自动归档功能**：
  - 新增 `archive_retries` 配置项（默认 `10`，设为 `0` 禁用），当种子汇报连续失败（重试）达到或超过设定次数时，自动将 `.torrent` 文件安全移动至 `archive_dir` 归档目录中
  - 移动到归档目录后调度器保持保种状态并继续重试汇报，不影响因网络抖动恢复后的上报
  - 种子在归档目录中若汇报成功，会自动将 `.torrent` 文件安全移回 `torrents_dir`
  - 方便用户集中管理与删除死种：用户可直接从 `archive_dir` 中删除 `.torrent` 文件，OpenPT 感知后会自动停止保种并清理状态
  - 存储层（Store）扩展为双目录（`torrents_dir` 与 `archive_dir`）协同监控与扫描，并对归档损坏文件增加重复解析短路，避免日志刷屏
  - Web UI 状态悬浮卡片增加归档存储位置指示，配置抽屉增加 `archive_retries` 展示

版本号 frontend/package.json -> 0.3.2
