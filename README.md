...
| `client` | string | 必填 | 要使用的客户端配置文件名，如 `qbittorrent-5.1.4.client` |
| `simultaneous_seed` | int | `1` | 同时保种的最大数量，调大时自动补满槽位 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `uploaded.strategy` | string | `"none"` | 上传量策略：`none`、`conservative_rate`、`configured_rate` |
| `uploaded.conservative_rate_bps` | int64 | `1024` | `conservative_rate` 策略的上传速率（字节/秒） |
| `uploaded.configured_rate_bps` | int64 | `0` | `configured_rate` 策略的上传速率（字节/秒） |
| `uploaded.min_rate_bps` | int64 | `0` | 全局上传速率随机区间最小值（字节/秒），`0` 表示不启用 |
| `uploaded.max_rate_bps` | int64 | `0` | 全局上传速率随机区间最大值（字节/秒），`> 0` 时启用随机速率 |
